...
import { deleteMember, TeamMemberInfo } from "../../../api/team";
import { setToastify, ToastStatus } from "../../../store/main/actions";
import { openDeleteMemberModal } from "../../../store/modals/actions";
...
const DeleteMemberModal: React.FC<{
  ...
  const openModal = (open) => {
    dispatch(openDeleteMemberModal(open));
  };

  const handleDelete = async () => {
    try {
      setLoading(true);
      const member = await deleteMember(user.user_id, teamId);
      dispatch(
        setToastify({
      ...
      onSuccess(member);
      dispatch(openDeleteMemberModal(false));
      // track by segment
      segmentTrack("Team Member Removed", {
        userId,
        teamId,
...
const mapStateToPros = (state) => {
  return {
    isOpenModal: state.modals?.isOpenDeleteMemberModal,
    userId: state.user?.id,
  };
};

export default connect(mapStateToPros)(DeleteMemberModal);
