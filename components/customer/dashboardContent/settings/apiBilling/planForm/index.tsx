...
function PlanForm({
  ...
  useEffect(() => {
    setAnnual(subscription?.x_plan?.interval === Interval.annual);
  }, [subscription]);
              ...
              >
                {({ active, checked }) => (
                  <>
                    ...
                    >
                      {description[router?.locale]?.map((text) => (
                        <p key={text}>{text}</p>
                      ))}
                    </div>
...
const mapStateToPros = (state) => {
  return {
    ...
    discountPercent: state.user?.x_subscription?.discount_percent_off ?? 100,
  };
};
