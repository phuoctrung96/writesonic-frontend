...
import { cancelXPlan, XSubscription } from "../../../../../api/credit_v2";
import { setToastify, ToastStatus } from "../../../../../store/main/actions";
import { openConfirmCancelXPlanModal } from "../../../../../store/modals/actions";
import { getBusinessSubscription } from "../../../../../store/user/actions";
...
function ConfirmCancelPlanModal({
  ...
  const openModal = (open) => {
    dispatch(openConfirmCancelXPlanModal(open));
  };
  ...
  const handleCancel = async () => {
    try {
      ...
      dispatch(getBusinessSubscription({ customerId: null }));
      if (mounted.current) {
      ...
      dispatch(openConfirmCancelXPlanModal(false));
    } catch (err) {
      if (mounted.current) {
        setIsCanceling(false);
      }
...
const mapStateToPros = (state) => {
  return {
    isOpenModal: state.modals?.isOpenConfirmCancelXPlanModal,
