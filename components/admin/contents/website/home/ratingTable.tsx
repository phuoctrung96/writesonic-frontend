...
import { getRatings } from "../../../../../api/admin/copy";
import { CreditHistoryInterfacePerType } from "../../../../../api/admin/creditHistory";
...
export default function RatingTable() {
  ...
  async function init(nextPage) {
    try {
      setIsLoading(true);
      const { items, total, page, size } = await getRatings(nextPage);
      if (!mounted.current) {
        return;
      }
      setTotal(total);
      ...
      items.forEach((item) => {
        ...
        Object.keys(item).forEach((key) => {
          if (key === "average_per_month") {
            updatedItem["total_rating_this_month"] = {
              value: item[key]["count"],
            };
            updatedItem["avg_rating_this_month"] = {
              value: item[key]["average"],
            };
          } else if (key === "average_per_week") {
            updatedItem["total_rating_this_week"] = {
              value: item[key]["count"],
            };
            updatedItem["avg_rating_this_week"] = {
              value: item[key]["average"],
            };
          } else if (key !== "id") {
            updatedItem[key] = {
