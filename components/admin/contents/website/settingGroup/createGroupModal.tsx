...
import { createContentTypeGroup } from "../../../../../api/admin/contentTypeGroup";
...
const CreateGroupModal: React.FC<{
  ...
  setIsOpenModal: any;
  onSuccess: () => void;
}> = ({ isOpenModal, setIsOpenModal, onSuccess }) => {
  ...
  const handleCreate = async () => {
    ...
    let imageSrc = null;
    try {
      imageSrc = await toBase64(selectedFile);
    } catch (err) {
      dispatch(
                    ...
                    <div className="mt-4">
                      <fieldset>
                        ...
                        <div className="mt-4 border-t border-b border-gray-200 divide-y divide-gray-200 h-44 overflow-y-auto px-4">
                          {contentTypes
                            .filter(({ title, content_type_group_id }) => {
                              if (content_type_group_id) {
                                return false;
                              }
                              if (searchKey) {
                                return title[locale]
